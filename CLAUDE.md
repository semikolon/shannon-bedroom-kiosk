...
**master = Bevy 0.14 (wgpu 0.20)** — production-stable on Shannon. Slices 3d-3g (daemon HA polling + Bevy ribbon-offer wiring + BlackoutTvPower + Ambient scene roots) **SHIPPED + LIVE-VERIFIED** 2026-05-21 ~23:48 CEST. AdapterInfo: Mali-T860 (Panfrost) OpenGL ES 3.1 Mesa 25.0.7-2 backend Gl. 75s soak load 0.17 CPU 45.6°C under freq caps. 62 tests (was 32 at session start). Uses vendored `wgpu-hal-0.21.1-mali-fix` 4-patch stack + `jpeg` Bevy feature + `zune-core = "=0.5.0-rc2"` pin + bg jpg resized to 2506×4000 (Mali 4096 texture limit).

**`bevy-upgrade` branch = Bevy 0.18.1 (wgpu 27.0.1)** — **LIVE-VERIFIED on Shannon Mali HW-GLES 2026-05-21 ~23:57 CEST.** 5+ min steady-state soak: AdapterInfo Mali-T860 Panfrost OpenGL ES 3.1, `backend: Gl`, CPU 44.4°C, load 0.16 stable.

**The actual root cause** of the night's Bevy 0.18 Mali blocker (after 3 wgpu-hal patches + Mesa env vars all hit the same `eglQueryDeviceStringEXT BAD_PARAMETER`): `bevy_render-0.18.1`'s wgpu dep is `default-features=false, features=["wgsl","dx12","metal","vulkan","naga-ir","fragile-send-sync-non-atomic-wasm"]` — **`gles` is NOT in the list**. Bevy's top-level `webgl2` feature maps to `wgpu/webgl` (WASM-only), NOT `wgpu/gles` (native Linux). Fix: direct `wgpu = { version = "27", default-features = false, features = ["gles", "wgsl"] }` dep in `Cargo.toml` — Cargo's feature-unification activates `gles` on bevy_render's wgpu dep transitively. Commit `930f57a` on `bevy-upgrade`.

The earlier three wgpu-hal vendor patches (X11 Wayland-veto + `WGPU_GL_PREFER_GLES` force_gles bind_api + robustness retry extended to BadParameter, commit `1e030fe`) live inside the GL backend code; without the `gles` feature they were never compiled in. Kept as defense-in-depth in `vendored/wgpu-hal-27.0.4-mali-fix/`.

**Recommendation**: merge `bevy-upgrade` → master when convenient. Unblocks Slice 3c step 3 (formal Xbox-styled chips via Bevy 0.15+ UI `border_radius`), brings ECS perf improvements + cosmic-text Unicode + modern gamepad event API. Master rollback path remains `git checkout master`.
