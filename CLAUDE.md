...
**Phase status (2026-05-24 evening)**: Phase 3 LIVE on Shannon (Lights, Music, Watch, Sleep tiles wired). Phase-7 spela-on-Shannon keystone shipped (Watch tile → Shannon spela-local + "Shannon TV" web-remote target). **Watch UI v1 LIVE** (Phase-7 rank-1 first slice — text-list browse of spela library, A=play, B=back; Phases 2-5 posters/now-playing/scrubber/search pending per `~/dotfiles/docs/shannon_watch_ui_design_2026_05_24.md`). **Lights latency target met** (~360-830ms end-to-end kiosk-button → physical lamp via daemon HTTP path, was ~2-2.5s subprocess path; see `~/dotfiles/system/shannon/README.md` § lights-daemon).
...
**🛑 2026-05-25 honest status — HW decode CPU still unsolved at 99% one core after 10 days + ~60 commits + 5 distinct pipeline shapes**. Detile-bypass theory disproven empirically (today's `playbin3 video-filter="capsfilter caps=video/x-raw,format=NV12"` didn't drop CPU). `v4l2slh264dec` IS engaged at rkvdec (lsof + frame counts confirm); the 99% is some invisible downstream consumer that NO pipeline-shape iteration has surfaced. **What hasn't been done and is the next step**: `perf top` / `pidstat -t` / `GST_DEBUG=GST_AUTOPLUG:6` to NAME the consuming function/thread/element — before any further architectural pivot. Full reckoning + diagnose-first plan: [`~/dotfiles/docs/shannon_kiosk_gpu_hwaccel_research_2026_05_18.md` § 6](~/dotfiles/docs/shannon_kiosk_gpu_hwaccel_research_2026_05_18.md) (updated 2026-05-25).

Current renderer state in spela-local (Path B+ primary, Path A v3 retained as code):

- **Path A v3 (manual gst-launch + souphttpsrc+hlsdemux)**: HW decode confirmed at file level (300/1349 frames in synthetic + warm-HLS tests) BUT cold-start fragile via /watch — hlsdemux throws `Internal data stream error` on Darwin's just-cold-started HLS regardless of readiness gating (3 revisions tried today). RETAINED as code but NOT invoked by default.
- **Path B+ (playbin3 + capsfilter NV12, primary)**: playbin3's adaptivedemux handles cold-start robustly; `v4l2slh264dec` engages at rkvdec (lsof confirms /dev/video2 held + frame counts at HW rate). **BUT 99% CPU sustained** — the capsfilter-at-decoder-output bypass (`video-filter` shape) didn't drop CPU as the detile-bypass research predicted. Real consumer of the 99% is **unidentified** as of 2026-05-25 evening; diagnostic (`perf top` / `pidstat -t` / `GST_DEBUG=GST_AUTOPLUG:6`) is the explicit next step before any further architectural change.
