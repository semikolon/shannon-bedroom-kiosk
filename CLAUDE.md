...
**Phase status (2026-05-26 ~01:00)**: Phase 3 LIVE on Shannon (Lights, Music, Watch, Sleep, Sensors, Buses tiles wired). Phase-7 spela-on-Shannon keystone shipped (Watch tile → Shannon spela-local + "Shannon TV" web-remote target). **HW-decode CPU resolved** — spela-local v4 cage+waylandsink renderer sustains 35-38% CPU (was 99% with playbin3+capsfilter+kmssink); see `~/dotfiles/docs/reflections/2026-05-25-rk3399-hw-decode-canonical-survey.md` § H.6 + `~/dotfiles/system/shannon/README.md` § "spela-local v4". **Watch UI v1 LIVE** (Phase-7 rank-1 first slice — text-list browse of spela library, A=play, B=back; Phases 2-5 posters/now-playing/scrubber/search pending per `~/dotfiles/docs/shannon_watch_ui_design_2026_05_24.md`). **Lights latency target met** (~360-830ms end-to-end kiosk-button → physical lamp via daemon HTTP path, was ~2-2.5s subprocess path; see `~/dotfiles/system/shannon/README.md` § lights-daemon).
...
Full HW-decode reasoning + iteration archaeology + diagnose-first plan + Fredrik's voice anchors: `~/dotfiles/docs/shannon_kiosk_gpu_hwaccel_research_2026_05_18.md` § 6 (canonical aggregator). Supporting: `~/dotfiles/docs/reflections/2026-05-25-shannon-hw-decode-archaeology.md` (595 lines, every attempt named) + `~/dotfiles/docs/reflections/2026-05-25-rkvdec-detile-bypass-research.md` (why detile bypass alone didn't fix CPU) + `~/dotfiles/docs/reflections/2026-05-25-hw-decode-research.md` + **`~/dotfiles/docs/reflections/2026-05-25-rk3399-hw-decode-canonical-survey.md`** (empirical floor IS achievable: Werner ~15% on Bookworm+Kwiboo; Trixie+Kwiboo has mpv 0.40 regression; flat-manual-pipeline = <30-min diagnostic). The Watch UI Phase 1.5 work (sidebar-stays + STARTING overlay + 30s debounce) is now named **Watch UI Legibility Pass** per *Self-decoding labels* directive — "Phase 1.5" is deprecated as an anonymous handle.
...
- **Watch UI Phase 3 NowPlaying — SHIPPED 2026-05-28** (commit `a94df3c`): new `src/now_playing.rs` module + `MenuLevel::NowPlaying` variant + `WatchSubmenu A-press` transitions to NowPlaying after dispatch + B returns + full-screen panel renders title + state (`▶ STREAMING` / `STARTING…` / `■ STOPPED`) + position (`M:SS / H:MM:SS`). Polls spela `/status` + `/api/position` every 2s; position interpolates between polls. 17 unit tests + cargo clippy/fmt clean. **Aarch64 cross-build + Shannon deploy pending** (gated on next Fredrik-at-TV session per existing soak protocol).
- **Watch UI Phase 2A Posters foundation — SHIPPED 2026-05-28** (commit `87536d3`): new `src/posters.rs` module with `PosterRegistry` + background downloader thread (reqwest blocking, 8s timeout, in-memory bytes-cache ~4.5 MB cap for current 57-entry library); main-thread systems request URLs from library snapshot → promote `Bytes` to `Handle<Image>` via `Image::from_buffer(JPEG)` → swap into `WatchPosterPreview` `ImageNode` on cursor moves. Visual win: selected film's poster shows right of the library text list. 4 unit tests + clean gates. **Phase 2B (5×2 grid layout sharing the same registry) is a follow-up** — deferred because 2A already gives most user-visible win at much lower layout-risk. Aarch64 cross-build + deploy pending alongside NowPlaying.
- **spela-renderer first slice — SHIPPED 2026-05-28** (commit `6254b1d`): new `src/bin/spela-renderer.rs` (~280 lines) + `gst-renderer` opt-in feature + Cross.toml gstreamer-libs delta. THE structural win: proper `tsdemux` pad-added signal handling for the audio branch (the `gst-launch d.` syntax limitation the design doc named). Includes Path-A pipeline (cage+waylandsink) mirroring `spela-local v4` empirically. Default cargo build (`shannon-kiosk` + `shannon-kiosk-actions`) still lean — gstreamer pulled only with `--features gst-renderer`. **Aarch64 cross-build in flight on Darwin background**; one-time Docker image rebuild (~5-10 min for the new arm64 gstreamer dev libs) before incremental builds resume. Path-A→B fallback + cage takeover logic are v2 slices.
