...
**Watch UI v1 KNOWN ISSUES (Fredrik's 2026-05-24 evening TV test — Phase 1.5 fix pass pending; see `~/dotfiles/docs/shannon_watch_ui_design_2026_05_24.md` § Phase 1.5)**:
- **Layout overlay**: WatchSubmenu's library list reuses the same x=130/200 pre-spawned `LightsSubmenuLabel/Icon` entity positions as Lights/Music — but those positions are UNDER the root menu sidebar. When `in_submenu=true`, `menu_render_system` HIDES the root menu so the submenu visually replaces it (works for Lights/Music with 3 short labels; broken for Watch's 10-entry list because user expected sidebar to STAY VISIBLE with library to the RIGHT). Fix: spawn separate `WatchListLabel/Icon` entities at x~600+ (right of sidebar) + special-case `is_watch` in the root-menu hide gate to KEEP root menu visible.
- **No immediate A-press feedback**: dispatching a Watch entry kicks spela-local subprocess fire-and-forget; spela-local then does /search + /play + waits for Darwin transcode cold-start (20-60s) + stops cage + launches gst pipeline. User sees nothing for that whole window → re-presses A thinking it missed. Fix: add `pending_watch_title: Option<String>` to `EngineRes` + a centered "STARTING `<title>`…" overlay Text entity visible while set; clear after N seconds OR on cage stop.
- **`WATCH_DEBOUNCE_MS=2000` was too short** (fixed in src but not yet deployed 2026-05-24 evening: bumped to 30000): two A presses 10s apart fired two parallel spela-local sessions which raced for /play; spela returned the SAME wrong result for both (`The Boys S05E08` for a query of `Ex.Machina...`) due to spela's `/play 1` stale-last-search-fallback bug. Bumping to 30s closes the symptom; the spela-side bug is separately tracked in `~/Projects/spela/TODO.md`.
