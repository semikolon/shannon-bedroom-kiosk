...
impl Inner {
    fn create(
        ...
        // NOTE: terminator (NONE) is appended per-attempt inside the loop below.

        // ---- BEGIN mali-fix patch (backport of wgpu PRs #7952 + #9153) ----
        // Retry-loop fallback for EGL_CONTEXT_OPENGL_ROBUST_ACCESS: try
        // Core → Ext → None on BadMatch | BadAttribute | BadConfig. The
        // original v0.21.1 code attempted CORE unconditionally and aborted
        // on failure, which breaks on Mali Midgard (Panfrost arch 5) where
        // the Mesa driver arch-gates `robust_buffer_access_behavior` to
        // arch >= 6 (Bifrost+). Fix backported from upstream wgpu:
        //   * #7952 (wgpu 27.0): introduces retry loop on BadAttribute
        //   * #9153 (wgpu 29.0): extends retry list to BadMatch | BadConfig
        // Surrounding v0.21.1 code is simpler than wgpu trunk's later
        // GL/GLES `bind_api` split — `egl.bind_api(...)` is called once at
        // the top of this function, so we don't need #9153's per-attempt
        // closure. Initial robustness selection preserves v0.21.1's
        // ANGLE-aware skip of Core (ANGLE rejects Core regardless of
        // extension support — see original comment below).
        #[derive(Copy, Clone)]
        enum Robustness {
            Core,
            Ext,
        ...
        // In fact, Angle does precisely that awful behavior, so we don't
        // try it there.
        let mut robustness = if version >= (1, 5) && !display_extensions.contains("EGL_ANGLE_") {
        ...
        let context = loop {
            let robustness_attributes: Vec<khronos_egl::Int> = match robustness {
                Some(Robustness::Core) => {
                ...
                None => {
                    // Note: we aren't trying
                    // `EGL_CONTEXT_OPENGL_ROBUST_ACCESS_BIT_KHR` because it's
                    // for desktop GL only, not GLES.
                    log::warn!("\tEGL context: -robust access");
                    vec![]
                }
            };

            let mut attempt_attributes = context_attributes.clone();
            attempt_attributes.extend(&robustness_attributes);
            attempt_attributes.push(khronos_egl::NONE);

            match egl.create_context(display, config, None, &attempt_attributes) {
                Ok(ctx) => break ctx,
                Err(e) => {
                    // Degrade on errors that indicate the driver doesn't
                    // support what we asked for. Mali Midgard hits BadMatch
                    // on Robust+Core; Intel HD 4000 (the original #9153
                    // reporter) hits BadConfig; BadAttribute was the only
                    // case handled pre-#9153.
                    ...
                    if degradable {
                        robustness = match robustness {
                            Some(Robustness::Core) => Some(Robustness::Ext),
                            Some(Robustness::Ext) => None,
                            None => {
                                return Err(crate::InstanceError::with_source(
                                    String::from(
                                        "unable to create GLES 3.x context \
                                         (all robustness levels exhausted)",
                                    ),
                                    e,
                                ));
                            }
