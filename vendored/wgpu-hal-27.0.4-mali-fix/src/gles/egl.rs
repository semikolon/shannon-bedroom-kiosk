...
impl crate::Instance for Instance {
    ...
    unsafe fn init(desc: &crate::InstanceDescriptor) -> Result<Self, crate::InstanceError> {
        ...
        // platform selection. Upstream prefers Wayland whenever
        // wl_display_connect(NULL) succeeds — which happens even with
        // WAYLAND_DISPLAY unset, because the Wayland client library
        // falls through to the default "wayland-0" socket in
        // $XDG_RUNTIME_DIR (cage sets that as part of its session).
        // On Mali T860 + Mesa Panfrost the Wayland EGL platform fails
        // with `eglQueryDeviceStringEXT EGL_BAD_PARAMETER` during
        // device enumeration → no GPU adapters → Bevy panics. The X11
        // EGL platform (via Xwayland under cage) works fine. Without
        // this veto, `env -u WAYLAND_DISPLAY` in the launcher isn't
        // sufficient. Backport of the same patch our wgpu-hal 0.21
        // vendor crate had (May 13, 2026).
